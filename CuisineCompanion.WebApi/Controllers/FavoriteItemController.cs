...
public class FavoriteItemController : MyControllerBase
{
    ...
    public IActionResult AddFavoriteItem(FavoriteItemCreationDto dto)
    {
        ...
        try
        {
            ...
            if (favorite.Authority != authority) return Ok(new ApiResponses { Code = -1, Message = "收藏夹权限错误" });

            if (favorite.IdCategory != idCategory) return Ok(new ApiResponses { Code = -1, Message = "收藏夹类型不符" });

            var favoriteItem =
                _db.FavoriteItems.FirstOrDefault(fi => fi.FavoriteId == dto.FavoriteId && fi.TId == dto.TId);
            if (favoriteItem is not null) return Ok(new ApiResponses { Code = -1, Message = "已存在" });
    ...
    {
        DbFlagsHelper.TryGetIdCategory(dto.Flag, out var idCategory);
        if (idCategory == 0) return Ok(new ApiResponses { Code = -1, Message = "参数错误" });
    ...
    public IActionResult RemoveFavoriteItems(FavoriteItemSeleteDto dto)
    {
        ...
        try
        {
            ...
            if (fis is null || fis.Count == 0) return Ok(new ApiResponses { Code = -1, Message = "无收藏" });
    ...
    {
        DbFlagsHelper.TryDeFlags(dto.Flag, out var _, out var idCategory);
        if (idCategory == 0) return Ok(new ApiResponses { Code = -1, Message = "参数错误" });
        ...
        try
        {
            ...
            if (favoriteItem is null) return Ok(new ApiResponses { Code = -1, Message = "无此收藏" });
