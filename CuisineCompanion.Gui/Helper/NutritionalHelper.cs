...
public static class NutritionalHelper
{
        ...
        decimal ans)
    {
        ...
        var nrv = new Dictionary<string, decimal>();
        {
            foreach (var (key, value) in allNutritional)
            {
                string unit = "";
                double rni = 0;
                if (!MainViewModel.TryGetNutritional(key, out unit)) continue;
                if (!MainViewModel.RNI?.TryGetValue(key, out rni) ?? false) continue;
                //   *100/ans  每100g可食部中营养素的含量
                dic[key] = new Tuple<decimal, string>(value * 100 / ans, unit);
                // 食品中某营养素的含量/该营养素的营养素参考值 *100%
                if (rni != 0)
                    nrv[key] = dic[key].Item1 * 100 / (decimal)rni;
            }
        }
        ...
        out List<NutrientContentViewModel> proteinContent)
    {
        ...
        foreach (var pieSegmentModel in list2) list1.Remove(pieSegmentModel);
    }
        ...
        out List<NutrientContentViewModel> proteinContent)
    {
        ...
        foreach (var (key, value) in dic)
        {
            ...
            if (key is "热量 kcal")
            {
                energy.Add(new Tuple<string, decimal>(key, value));
                energy.Add(new Tuple<string, decimal>("热量 kJ", UnitHelper.ConvertUnit(value, "kcal", "kJ")));
                continue;
            }
            ...
            {
                UnitHelper.ConvertToClosestUnit(value, unit, out var value2, out var unit2);
            ...
            {
                Value = UnitHelper.ConvertToBaseUnit(value, unit),
                Item = key,
                Color = key switch
                {
